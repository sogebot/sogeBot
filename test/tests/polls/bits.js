...
describe('Polls - bits', () => {
  ...
  describe('Voting full workflow', () => {
    ...
    it(`!vote should return correct vote status`, async () => {
      ...
      assert.strictEqual(r[1].response, `#vote1 - Lorem - 0 ${getLocalizedName(0, 'systems.polls.votes')}, 0.00%`);
      assert.strictEqual(r[2].response, `#vote2 - Ipsum - 0 ${getLocalizedName(0, 'systems.polls.votes')}, 0.00%`);
      assert.strictEqual(r[3].response, `#vote3 - Dolor Sit - 0 ${getLocalizedName(0, 'systems.polls.votes')}, 0.00%`);
    });
    ...
    it(`!vote should return correct vote status`, async () => {
      ...
      const r = await polls.main({ sender: owner, parameters: ''  });
      assert.strictEqual(r[0].response, '$sender, current status of poll "Lorem Ipsum?":');
      assert.strictEqual(r[1].response, `#vote1 - Lorem - 100 ${getLocalizedName(100, 'systems.polls.votes')}, 50.00%`);
      assert.strictEqual(r[2].response, `#vote2 - Ipsum - 100 ${getLocalizedName(100, 'systems.polls.votes')}, 50.00%`);
      assert.strictEqual(r[3].response, `#vote3 - Dolor Sit - 0 ${getLocalizedName(0, 'systems.polls.votes')}, 0.00%`);
    });
