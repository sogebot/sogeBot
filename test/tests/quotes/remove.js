...
const tests = [
  ...
  { sender: owner, parameters: '-id a', id: 'a', shouldFail: false, exist: false },
  { sender: owner, parameters: '-id 9999', id: 9999, shouldFail: false, exist: false },
  ...
  for (const test of tests) {
    describe(test.parameters, async () => {
      ...
      it('Run !quote remove', async () => {
        global.systems.quotes.remove({ sender: test.sender, parameters: test.parameters.replace('$id', id), command: '!quote remove' });
      });
      if (test.shouldFail) {
