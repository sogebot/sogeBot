...
describe('Top - !top subage - @func1', () => {
  ...
  it ('Add 10 users into db and last user will don\'t have any subage', async () => {
    for (let i = 0; i < 10; i++) {
      await AppDataSource.getRepository(User).save({
        ...
        userName:     'user' + i,
        isSubscriber: true,
        subscribedAt: new Date(Date.now() - (HOUR * i)).toISOString(),
      });
    }
  });
  ...
  it ('Add user with long subage but not subscriber', async () => {
    await AppDataSource.getRepository(User).save({
      ...
      userName:     'user11',
      isSubscriber: false,
      subscribedAt: new Date(Date.now() - (HOUR * 24 * 30)).toISOString(),
    });
  });
  ...
  it('run !top subage and expect correct output', async () => {
    ...
    const dates = [];
    for (let i = 0; i < 10; i++) {
      dates.push(`${dayjs.utc(Date.now() - (HOUR * i)).format('L')} (${dayjs.utc(Date.now() - (HOUR * i)).fromNow()})`);
    }
