...
describe('User joins duel twice', () => {
  ...
  it('user 1 is challenging with 50 points', async () => {
    const responses = await duel.main({ sender: user1, parameters: '50', command });
...
describe('Duel is on cooldown without bypass by mods', () => {
  ...
  it('add points for users', async () => {
    await getRepository(User).save({ userId: user1.userId, username: user1.username, points: 100 });
    await getRepository(User).save({ userId: user.owner.userId, username: user.owner.username, points: 100 });
    await getRepository(User).save({ userId: user.mod.userId, username: user.mod.username, points: 100 });
  });
  ...
  it('owner is challenging and should be on cooldown', async () => {
    const responses = await duel.main({ sender: user.owner, parameters: '50', command });
    assert(responses[0].response === '$sender, you cannot use !duel for 10 minutes.', JSON.stringify({responses}));
  });

  it('mod is challenging and should be on cooldown', async () => {
    const responses = await duel.main({ sender: user.mod, parameters: '50', command });
    assert(responses[0].response === '$sender, you cannot use !duel for 10 minutes.', JSON.stringify({responses}));
  });
});
