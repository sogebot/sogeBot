...
class Songs {
  constructor () {
    if (global.commons.isSystemEnabled(this)) {
      ...
      global.panel.registerSockets({
        self: this,
        expose: ['getCurrentVolume', 'sendPlaylist', 'setTrim', 'sendConfiguration', 'banSong'],
        finally: this.sendPlaylist
      })
    }
  }
  ...
  async sendPlaylist (self, socket) {
    let playlist = await global.db.engine.find('playlist')
  ...
  async banCurrentSong (self, sender) {
    ...
    if (update > 0) {
      ...
      self.sendPlaylist(self, global.panel.io)
    }
  }
