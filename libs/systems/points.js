...
Points.prototype.setPoints = function(user, text) {
    if (text.length < 1) {
    ...
    var user   = text.split(" ")[0],
        points = parseInt(text.replace(user,'').trim()),
        self   = this;
...
Points.prototype.givePoints = function(user, text) {
    ...
    var user2  = text.split(" ")[0],
        points = parseInt(text.replace(user2,'').trim()),
        self   = this;
    ...
    database.findOne({ username: user.username }, function (err, item) {
        if (typeof item === undefined || item === null) {
            ...
            database.update({username: user.username}, {$set: {points: parseInt(item.points) - points}}, {});
...
Points.prototype.getPointsFromUser = function(user, text) {
    if (text.length < 1) {
        global.client.action(global.configuration.get().twitch.owner, 'Points error: You need to specify user');
        return;
    }
...
Points.prototype.allPoints = function(user, text) {
    ...
    database.find({ isOnline: true }, function (err, items) {
        items.forEach(function(e, i, ar) {
            database.update({username: e.username}, {$set: {points: parseInt(e.points) + points}}, {});
        });
...
Points.prototype.rainPoints = function(user, text) {
    ...
    database.find({ isOnline: true }, function (err, items) {
        items.forEach(function(e, i, ar) {
            ...
            database.update({username: e.username}, {$set: {points: parseInt(e.points) + random}}, {});
        });
        global.client.action(global.configuration.get().twitch.owner, 'I just added 0-' + points + ' Points to all users');
    });
}

Points.prototype.addPoints = function(user, text) {
    if (text.length < 1) {
        global.client.action(global.configuration.get().twitch.owner, 'Points error: You need to specify user');
        return;
    }
...
Points.prototype.removePoints = function(user, text) {
    if (text.length < 1) {
        global.client.action(global.configuration.get().twitch.owner, 'Points error: You need to specify user');
        return;
    }
