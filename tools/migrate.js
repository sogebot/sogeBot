...
const runIndexing = async(retry = false) => {
  // indexing
  console.info(('-').repeat(56));
  ...
  if (config.database.type === 'mongodb') {
    ...
    for (const k of Object.keys(collections)) {
      await global.db.engine.getClient().collection(collections[k].name).dropIndexes();
      await global.db.engine.getClient().createCollection(collections[k].name);
    }
  }
  try {
