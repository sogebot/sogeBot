...
class Roulette extends Game {
  ...
  async main (opts) {
    ...
    sendMessage(global.translate('gambling.roulette.trigger'), opts.sender);
    if (isBroadcaster(opts.sender)) {
      setTimeout(() => sendMessage(global.translate('gambling.roulette.broadcaster'), opts.sender), 2000);
      return;
    }

    if (isMod) {
      setTimeout(() => sendMessage(global.translate('gambling.roulette.mod'), opts.sender), 2000);
      return;
    }
    ...
    setTimeout(async () => {
      ...
      sendMessage(isAlive ? global.translate('gambling.roulette.alive') : global.translate('gambling.roulette.dead'), opts.sender);
    }, 2000);
  }
}
