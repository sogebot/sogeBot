...
export class tipsExchangeRates1584106270435 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(`CREATE TABLE "user_tip" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "amount" float NOT NULL, "sortAmount" float NOT NULL, "exchangeRates" text NOT NULL, "currency" varchar NOT NULL, "message" text NOT NULL, "tippedAt" bigint NOT NULL DEFAULT (0), "userUserId" integer, CONSTRAINT "FK_36683fb221201263b38344a9880" FOREIGN KEY ("userUserId") REFERENCES "user" ("userId") ON DELETE CASCADE ON UPDATE CASCADE)`, undefined);
    ...
    for (const tip of tipsWithExchangeRates) {
      ...
      await queryRunner.query(
        `INSERT INTO user_tip (amount, sortAmount, exchangeRates, currency, message, tippedAt, userUserId)
          VALUES ("${tip.amount}", "${tip.sortAmount}", '${JSON.stringify(tip.exchangeRates)}', "${tip.currency}", "${tip.message.replace(/\"/g, '\\"')}", "${tip.tippedAt}", "${tip.userUserId}")
        `, undefined);
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(`CREATE TABLE "user_tip" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "amount" float NOT NULL, "sortAmount" float NOT NULL, "currency" varchar NOT NULL, "message" text NOT NULL, "tippedAt" bigint NOT NULL DEFAULT (0), "userUserId" integer, CONSTRAINT "FK_36683fb221201263b38344a9880" FOREIGN KEY ("userUserId") REFERENCES "user" ("userId") ON DELETE CASCADE ON UPDATE CASCADE)`, undefined);
    ...
    for (const tip of tips) {
      ...
      await queryRunner.query(
        `INSERT INTO user_tip (amount, sortAmount, currency, message, tippedAt, userUserId)
          VALUES ("${tip.amount}", "${tip.sortAmount}", "${tip.currency}", "${tip.message}", "${tip.tippedAt}", "${tip.userUserId}")
        `, undefined);
    }
  }
}
