...
const columnUserIdWithoutDefault = new TableColumn({ type: 'number', name: 'userId' });
const columnTypeWithoutDefault = new TableColumn({ type: 'string', name: 'type' });
...
export class dashboardPerTypeAndUser1584981805762 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      const columnUserId = new TableColumn({ type: 'number', default: broadcasterId, name: 'userId' });
      const columnType = new TableColumn({ type: 'string', default: '\'admin\'', name: 'type' });
