...
export class moveAlertMediaToGallery1643061611856 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const item of items) {
      const ids = (await queryRunner.query(`SELECT id FROM alert_media WHERE b64data=?`, [item.b64data])).map((o: any) => o.id);
      ...
      if (item.b64data.length > 0) {
        try {
          ...
          const id = shortid();
          await queryRunner.query(`INSERT INTO gallery(id, type, data, name, folder) VALUES(?, ?, ?, ?, ?)`,
    ...
    for (const type of ['cheer', 'command_redeem', 'follow', 'host', 'raid', 'resub', 'reward_redeem', 'sub', 'subcommunitygift', 'subgift', 'tip']) {
      ...
      for (const alert of alerts) {
        ...
        await queryRunner.query(`UPDATE \`alert_${type}\` SET imageId=?, soundId=? WHERE id=?`, [ imageId, soundId, alert.id ]);
      }
    }
