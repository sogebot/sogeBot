...
export class changeGoalTimestamps1651671003484 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "goal_group" ADD "createdAt" character varying NOT NULL`);
    await queryRunner.query(`ALTER TABLE "goal" DROP COLUMN "timestamp"`);
    await queryRunner.query(`ALTER TABLE "goal" ADD "timestamp" character varying`);
    ...
    for (const item of goal) {
      ...
      await queryRunner.query(
        `INSERT INTO "goal"(${keys.map(o => `"${o}"`).join(', ')}) values (${keys.map((o, i) => `$${i+1}`).join(', ')})`,
        keys.map(key => item[key]),
      );
    }
  }
