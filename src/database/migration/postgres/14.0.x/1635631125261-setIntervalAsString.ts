...
export class setIntervalAsString1635631125261 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "goal" ADD "interval" character varying NOT NULL DEFAULT 'hour'`);
    for (const item of items) {
      ...
      await queryRunner.query(
        `INSERT INTO "goal"(${keys.map(o => `"${o}"`).join(', ')}) values (${keys.map((o, idx) => `$${idx+1}`).join(', ')})`,
