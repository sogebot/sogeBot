...
class Keywords extends System {
  constructor() {
    super();
    this.addMenu({
      category: 'commands', name: 'keywords', id: 'commands/keywords', this: this, scopeParent: this.scope(),
    });
  }
  ...
  async removeOne(req: any) {
    const al = await Keyword.findOneBy({ id: req.params.id });
    if (al) {
      await al.remove();
    }
  }
  @Post('/', { customEndpoint: '/systems/groups/keywords' })
  saveOneGroup(req: any) {
    return KeywordGroup.create(req.body).save();
  }
  @Get('/', {
    scope:          'read',
    customEndpoint: '/systems/groups/keywords',
  })
  findAllGroups() {
