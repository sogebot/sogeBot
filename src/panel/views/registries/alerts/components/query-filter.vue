<template>
  <b-form-group class="m-0">
    <b-input-group v-if="!noInput">
      <template #prepend>
        <b-button @click="editationMode = !editationMode" :pressed="editationMode">
          <fa icon="cog" fixed-width/>
        </b-button>
      </template>
      <b-form-textarea v-model="stringifiedFilter" readonly/>
    </b-input-group>
          ...
          <b-row class="align-items-end" no-gutters>
            <b-col cols="auto" class="pr-2">
              ...
              <b-select v-model="_filter.operator">
                <b-select-option value="and">and</b-select-option>
                <b-select-option value="or">or</b-select-option>
                <b-select-option value="and not">and not</b-select-option>
                <b-select-option value="or not">or not</b-select-option>
              </b-select>
            </b-col>
            <b-col cols="auto">
              <label-inside>{{translate('registry.alerts.filter.rule')}}</label-inside>
              <b-select v-model="addRuleType[0]" class="w-auto">
                <b-select-option v-for="rule of rules" :value="rule[0]" v-bind:key="rule[0]">{{rule[0]}}</b-select-option>
              </b-select>
              <b-button @click="_filter.items.push({
                type: addRuleType[0],
                typeof: getRuleType(addRuleType[0]),
                comparator: 'eq',
                value: getRuleDefaultValue(addRuleType[0]),
              })">{{translate('registry.alerts.filter.addRule')}}</b-button>
              <b-button @click="_filter.items.push(null)">{{translate('registry.alerts.filter.addGroup')}}</b-button>
            </b-col>
            <b-col v-if="deletable" cols="auto" class="align-self-end">
              <b-button variant="outline-danger" class="border-0" @click="deleteGroup"><fa icon="trash" fixed-width/></b-button>
            </b-col>
          </b-row>

          <b-card no-body class="p-2" v-if="_filter.items.length > 0">
            <template v-for="(item, index) in _filter.items">
              <b-card-text v-if="item !== null && typeof item.items === 'undefined'"  v-bind:key="'item' + index" class="p-0">
                <b-row class="align-items-end" no-gutters>
                  <b-col>
                    <b-form-group class="m-0">
                      <b-row class="align-items-end" no-gutters>
                        <b-col style="position: relative; top: -7px;">{{item.type}}</b-col>
                        <b-col cols="4" class="pr-2">
                          ...
                          <b-select v-model="item.comparator">
                            <b-select-option value="is-even" v-if="getRuleType(item.type) === 'number'">{{translate('registry.alerts.filter.isEven')}}</b-select-option>
                            <b-select-option value="is-odd" v-if="getRuleType(item.type) === 'number'">{{translate('registry.alerts.filter.isOdd')}}</b-select-option>
                            <b-select-option value="lt" v-if="getRuleType(item.type) !== 'string'">{{translate('registry.alerts.filter.lessThan')}}</b-select-option>
                            <b-select-option value="lt-eq" v-if="getRuleType(item.type) !== 'string'">{{translate('registry.alerts.filter.lessThanOrEqual')}}</b-select-option>
                            <b-select-option value="co" v-if="getRuleType(item.type) === 'string'">{{translate('registry.alerts.filter.contain')}}</b-select-option>
                            <b-select-option value="eq">{{translate('registry.alerts.filter.equal')}}</b-select-option>
                            <b-select-option value="neq">{{translate('registry.alerts.filter.notEqual')}}</b-select-option>
                            <b-select-option value="pr">{{translate('registry.alerts.filter.present')}}</b-select-option>
                            <b-select-option value="includes" v-if="getRuleType(item.type) === 'string'">{{translate('registry.alerts.filter.includes')}}</b-select-option>
                            <b-select-option value="gt" v-if="getRuleType(item.type) !== 'string'">{{translate('registry.alerts.filter.greaterThan')}}</b-select-option>
                            <b-select-option value="gt-eq" v-if="getRuleType(item.type) !== 'string'">{{translate('registry.alerts.filter.greaterThanOrEqual')}}</b-select-option>
                          </b-select>
                        </b-col>
                        <b-col class="pr-2">
                          <template v-if="!['is-even', 'is-odd', 'pr'].includes(item.comparator)">
                            <label-inside>{{translate('registry.alerts.filter.value')}}</label-inside>
                            <b-select v-model="item.value" v-if="getRuleType(item.type) === 'tier'">
                              <b-select-option value="Prime">Prime</b-select-option>
                            ...
                            <b-form-input type="number" v-model.number="item.value" v-else-if="getRuleType(item.type) === 'number'"/>
                            <b-form-input v-model="item.value" v-else-if="item.comparator === 'includes'" v-b-tooltip.hover.noninteractive :title="translate('registry.alerts.filter.valueSplitByComma')" />
                            <b-form-input v-model="item.value" v-else />
                          </template>
                        </b-col>
                        <b-col cols="auto" class="align-self-end">
                          <b-button variant="outline-danger" class="border-0" @click="deleteItem(index)"><fa icon="trash" fixed-width/></b-button>
                        </b-col>
                      </b-row>
                    </b-form-group>
                  </b-col>
              ...
              <b-card-text v-else v-bind:key="'item' + index">
                <query-filter
