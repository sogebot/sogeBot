    ...
    <div class="col-12" v-for="(filter, index) of cFilters" :key="filter.comparator + filter.type + filter.value">
      <div class="row">
        <div class="form-group col-md-4">
          <select v-model="filter.type" class="form-control" @input.passive="setComparator(filter)">
        ...
        <div class="form-group col-md-3">
          <select v-model="filter.comparator" class="form-control">
            <option value="<" v-if="isNumber(filter.type)">{{ translate('core.permissions.isLowerThan') }}</option>
            <option value="<=" v-if="isNumber(filter.type)">{{ translate('core.permissions.isLowerThanOrEquals') }}</option>
            <option value="==">{{ translate('core.permissions.equals') }}</option>
            <option value=">=" v-if="isNumber(filter.type)">{{ translate('core.permissions.isHigherThanOrEquals') }}</option>
            <option value=">" v-if="isNumber(filter.type)">{{ translate('core.permissions.isHigherThan') }}</option>
          </select>
        </div>
        <div class="form-group col-md-4">
          <input v-if="isNumber(filter.type)" v-model.lazy.number="filter.value" type="number" class="form-control"/>
          <input v-else v-model.lazy="filter.value" type="string" class="form-control"/>
        </div>
