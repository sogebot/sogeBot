          ...
          b-card-text
            span(v-if="nonWatchedVariablesCount > 0")
              ...
              button(@click="addToWatch(selectedVariable)").btn.btn-block.btn-primary
                | {{ translate('widgets.randomizer.add-variable-into-watchlist') }}
            span(v-else)
              div(v-html="translate('widgets.randomizer.no-custom-variable-found')").alert.alert-warning
</template>
...
export default {
  ...
  data: function () {
    return {
      ...
      socket: getSocket('/widgets/randomizer'),
      draggingItem: null,
      }
  },
